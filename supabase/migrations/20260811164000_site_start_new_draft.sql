...
-- Bug crítico achado em teste ponta a ponta: site_publish_configuration
-- nunca cria um rascunho novo — depois de publicar, a configuração fica
-- travada pra sempre (não tem outra RPC que abra um rascunho seguinte).
-- Isso trava QUALQUER tenant assim que publicar pela primeira vez, não só
-- o piloto. Esta RPC clona o último rascunho PUBLISHED (todas as tabelas
-- filhas, com IDs novos) para um rascunho DRAFT novo — o dono continua
-- editando exatamente o que publicou, não perde nada.
create function public.site_start_new_draft(
...
begin
  ...
  -- Idempotente: se já existe um rascunho aberto, não duplica — só devolve
  -- a configuração carregada normalmente.
