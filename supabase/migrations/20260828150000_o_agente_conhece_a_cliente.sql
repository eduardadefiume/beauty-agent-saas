-- O agente passa a saber com quem está falando.
--
-- As 286 fichas foram importadas do histórico real do salão e provadas há dois
-- dias. Só que ficha guardada e ficha usada são coisas diferentes: até agora o
-- agente atendia toda cliente como se fosse a primeira vez. A Renata que faz
-- morena iluminada a cada 60 dias há dois anos era tratada igual a alguém que
-- nunca pisou lá.
--
-- É isso que muda aqui, em duas frentes.
--
-- 1. `client` NO BLOCO VOLÁTIL: quem ela é, o cabelo dela, a química que já
--    fez, o que ela costuma fazer e de quanto em quanto tempo, e as três
--    últimas visitas com duração e valor REAIS. Vai no volátil porque é de uma
--    pessoa só; no cacheado ele vazaria de conversa para conversa.
--
--    `isKnown` é a chave que separa os dois fluxos que a dona descreveu:
--    conhecida, o agente segue com o que já sabe; nova, ele investiga e marca
--    o teste de mecha junto com o procedimento.
--
-- 2. `statusArts` NO BLOCO ESTÁVEL: as artes de promoção que chegaram ao
--    salão. É informação do salão, igual para toda conversa, então entra no
--    prefixo cacheado e não custa por mensagem.
--
--    E aqui mora um cuidado que quase virou bug: NADA que mude a toda hora
--    pode entrar nesse bloco. `times_seen` sobe cada vez que uma cliente manda
--    a mesma arte; se ele entrasse aqui, o prefixo mudaria de bytes e o cache
--    cairia em silêncio -- o custo por mensagem voltaria ao que era antes sem
--    ninguém perceber. Por isso entra `confirmadaPorMaisDeUma`, que muda no
--    máximo uma vez na vida da arte.
--
-- CADÊNCIA VERSUS RITMO. `cadenceDays` é de quanto em quanto tempo ELA faz
-- aquilo, não uma regra do salão. `daysSince` e `cycleRatio` dizem onde ela
-- está nesse ciclo: 1.0 é a hora dela, 1.5 é atrasada. E `cadenceConfidence`
-- vem junto de propósito -- cadência tirada de duas visitas é chute, e o
-- agente precisa saber a diferença entre "está na hora" e "eu acho".
...
begin
  ...
  -- As artes que o salão colocou no ar. Ordenadas por data de primeira
  -- aparição -- determinístico, sem relógio, sem contador: o bloco cacheado
  -- precisa ser byte a byte igual entre conversas.
  v_artes := coalesce((
    select jsonb_agg(jsonb_build_object(
             'id', a.id,
    ...
    v_cliente := jsonb_build_object(
      ...
      'procedures', coalesce((
        select jsonb_agg(jsonb_build_object(
                 'family', pr.family,
                 'label', pr.label,
                 'timesDone', pr.times_done,
                 'lastDoneAt', pr.last_done_at,
                 'cadenceDays', pr.cadence_days,
                 'cadenceConfidence', pr.cadence_confidence,
                 'daysSince', case when pr.last_done_at is null then null
                   else (current_date - pr.last_done_at) end,
                 -- 1.0 = está na hora dela. Acima de 1 = atrasada. Comparar o
                 -- atraso com a cadência DELA, e não com um número fixo, é o
                 -- que evita cobrar quem faz a cada 90 dias como se fizesse a
                 -- cada 30.
                 'cycleRatio', case
                   when pr.last_done_at is null or coalesce(pr.cadence_days, 0) = 0 then null
                   else round((current_date - pr.last_done_at)::numeric / pr.cadence_days, 2)
                 end
  ...
  v_volatil := jsonb_build_object(
    ...
    'history', coalesce((
      select jsonb_agg(
        ...
        -- Citação: numa resposta a status a Cloud API entrega o id do que foi
        -- citado e nada mais. Serve para o agente saber QUE ela respondeu
        -- alguma coisa, mesmo sem saber o quê -- e aí olhar statusArts.
...
comment on function app.build_agent_context(uuid, integer) is
  'Contexto do agente em dois blocos. Estavel (cacheavel, do salao): catalogo, horario, equipe e as artes de promocao no ar. Volatil (desta conversa): a ficha da cliente com cadencia e ritmo, o historico com leitura de imagem e audio, e a janela de 24h.';
