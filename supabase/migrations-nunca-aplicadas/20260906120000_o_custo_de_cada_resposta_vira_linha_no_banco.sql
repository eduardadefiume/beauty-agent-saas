-- O CUSTO DE CADA RESPOSTA DO AGENTE VIRA LINHA NO BANCO
--
-- Ate aqui o `usage` que a Anthropic devolve era somado dentro da funcao,
-- escrito no console e jogado fora. Nao existia uma linha no banco dizendo
-- quanto custou atender uma cliente.
--
-- Isso importa porque a proxima decisao do negocio e de preco de assinatura, e
-- preco decidido sobre um numero que ninguem mede e chute. Sem medida nao da
-- para saber se uma mudanca economizou ou so mudou de lugar, nem se um salao
-- especifico esta caro, nem se a conta cresce com o numero de clientes ou com o
-- numero de mensagens -- que sao problemas diferentes com solucoes diferentes.
--
-- A TABELA DE PRECOS FICA NO BANCO, e nao no codigo da funcao. Quando a
-- Anthropic mexer no preco, uma linha aqui reprecifica a leitura do historico
-- inteiro, sem redeploy e sem numero velho escondido no meio.
...
-- O total sozinho nao diz o que fazer com ele; a divisao entre escrita de
-- cache, leitura de cache, entrada sem cache e saida diz.
