...
const components: ComponentRecord[] = [
  {
    ...
    icon: Database,
    evidence:
      'A operação deve carregar somente o tenant autorizado pela sessão. A prova final depende dos testes negativos de RLS no Supabase.',
  },
  {
    ...
    icon: ShieldCheck,
    evidence:
      'Nenhuma credencial é compartilhada entre tenants. A conexão WhatsApp só pode ser exibida após a vinculação do canal do próprio negócio.',
  },
  {
    ...
    icon: Users,
    evidence:
      'Contatos, canais e consentimentos serão segregados por tenant. Nenhum contato de outro negócio pode aparecer nesta área.',
  },
  {
    ...
    icon: Radio,
    evidence:
      'A inbox só exibirá mensagens do canal associado ao negócio atual, após a validação do webhook e das políticas de acesso.',
  },
];
...
function StatusPill({
  ...
  tone: 'olive' | 'amber' | 'ink';
  children: React.ReactNode;
}) {
  const className =
    tone === 'amber'
      ? `${styles.statusPill} ${styles.statusPillAmber}`
      : `${styles.statusPill} ${styles.statusPillOlive}`;
...
export default function DashboardPilotoWilliamPage() {
        ...
        if (!response.ok) {
          throw new Error(
            response.status === 401
              ...
              : (body.error ?? 'TENANT_CONTEXT_UNAVAILABLE')
          );
        }
        return body;
      })
    ...
    return (
      <main className={styles.accessState} aria-busy="true">
        <section className={styles.accessCard}>
          ...
          <p>
            Estamos confirmando o negócio autorizado para esta sessão antes de abrir os dados
            operacionais.
          </p>
        </section>
      </main>
    ...
    return (
      <main className={styles.accessState}>
        <section className={styles.accessCard}>
          ...
          <p>
            {workspaceError ??
              'Esta sessão não possui um negócio autorizado para a área operacional.'}
          </p>
  ...
  const tenantName =
    workspace?.tenantName ?? (workspaceLoading ? 'Carregando negócio' : 'Negócio não disponível');
    ...
    <div className={styles.appShell}>
      <aside className={`${styles.sidebar} ${mobileNavOpen ? styles.sidebarOpen : ''}`}>
        ...
        <div className={styles.sidebarContext}>
          ...
          <span className={styles.contextEnv}>
            <CircleDot size={9} fill="currentColor" /> contexto autorizado
          </span>
        </div>
        ...
        <header className={styles.topbar}>
          ...
          <div className={styles.topbarActions}>
            <div className={styles.liveIndicator}>
              <span className={styles.liveDot} /> tenant autorizado
            </div>
            <button
            ...
            <button className={styles.avatarButton} aria-label="Perfil de Duda">
              D
            </button>
          </div>
        </header>
        ...
        <div className={styles.contentWrap}>
          <section className={styles.introRow} id="overview">
            <div className={styles.introCopy}>
              <div className={styles.overline}>
                <span className={styles.overlineLine} /> relatório de prontidão operacional
              </div>
              <div className={styles.dossierSignature}>
                <div>
                  <strong>{tenantName.toUpperCase()} / OPERAÇÃO</strong>
                  <span>caderno operacional · contexto autenticado</span>
                </div>
              </div>
              ...
              <p className={styles.introLede}>
                Acompanhe a inbox, os agendamentos, as confirmações e a configuração do seu próprio
                negócio — sem acesso a dados de outros tenants.
              </p>
              <div className={styles.introMeta}>
                <span>
                  <Clock3 size={14} /> status DB: {statusInfo.databaseStatus}
                </span>
                <span>
                  <GitBranch size={14} /> tenant: {workspace?.tenantSlug ?? 'indisponível'}
                </span>
              </div>
              <div className={styles.proofRail}>
                <div>
                  <span>leitura de evidência</span>
                  <strong>{statusInfo.allowlistCount} número autorizado</strong>
                </div>
                <div className={styles.proofPending}>
                  <span className={styles.proofDot} />
                  <div>
                    <span>eventos capturados</span>
                    <strong>{inboxEvents.length} na inbox</strong>
                  </div>
                </div>
              </div>
            </div>
            <div className={styles.heroFrame}>
              ...
              <div className={styles.heroFrameLabel}>
                <span>01</span>
                <span>base operacional</span>
              </div>
              <div className={styles.heroStamp}>OP / 001</div>
              <div className={styles.heroProofTag}>
                <span className={styles.liveDot} /> operação / tenant
              </div>
            </div>
          </section>
          ...
          <section className={styles.statusStrip} aria-label="Resumo do estado atual">
            <div className={styles.statusPrimary}>
              ...
              <div>
                <span className={styles.eyebrow}>estado atual</span>
                <strong>{statusInfo.statusText}</strong>
              </div>
            </div>
            ...
            <div className={styles.statusStat}>
              <span className={styles.statLabel}>EVENTOS INBOX</span>
              <strong>{inboxEvents.length}</strong>
              <span className={styles.statFoot}>app.inbox_events</span>
            </div>
            <button className={styles.statusAction} onClick={() => handleNav('timeline')}>
              ver timeline <ArrowUpRight size={16} />
            </button>
          </section>
          ...
          <section className={styles.milestoneSection} aria-labelledby="milestones-title">
            <div className={styles.milestoneHead}>
              <div>
                <span className={styles.eyebrow}>linha de avanço</span>
                <h2 id="milestones-title">Cinco movimentos até a agenda</h2>
              </div>
              <div className={styles.milestoneCounter}>
                <strong>03</strong>
                <span>marcos atravessados</span>
              </div>
            </div>
                ...
                return (
                  <div className={`${styles.milestone} ${stateClass}`} key={milestone.number}>
                    <div className={styles.milestoneDot}>
                      {milestone.state === 'done' ? <Check size={13} /> : milestone.number}
                    </div>
          ...
          <div className={styles.sectionGrid} id="integrations">
            <section className={styles.componentsPanel}>
              <SectionHeading
                eyebrow="01 / integridade da base"
                title="O que já está de pé"
                copy="Base operacional estruturada para o seu negócio, com isolamento por tenant e evidências de ativação por módulo."
              />
                ...
                {components.map((component, index) => {
                  ...
                  const iconToneClass =
                    component.tone === 'amber'
                      ? styles.componentIconAmber
                      : styles.componentIconOlive;
                  return (
                    <button
                    ...
                    >
                      ...
                      <span className={`${styles.componentIcon} ${iconToneClass}`}>
                        <Icon size={17} />
                      </span>
            ...
            <aside className={styles.evidenceCard} id="evidence">
              <div className={styles.evidenceImage}>
                <img src={detailImage} alt="Detalhe de estação de trabalho do salão" />
                <span className={styles.imageCaption}>matéria / precisão / cuidado</span>
              </div>
              <div className={styles.evidenceContent}>
                <div className={styles.evidenceTopline}>
                  <span className={styles.eyebrow}>próxima prova</span>
                  <span className={styles.evidenceIndex}>02 / 04</span>
                </div>
                <h3>
                  Uma mensagem.
                  <br />
                  <em>Um evento real.</em>
                </h3>
                <p>
                  Conecte o número WhatsApp do seu negócio para iniciar a ingestão segura das
                  conversas na inbox operacional.
                </p>
                <button
                  className={styles.inkButton}
                  onClick={() => window.location.assign('/whatsapp')}
                >
                  abrir conversas ao vivo <ArrowUpRight size={16} />
                </button>
              </div>
            </aside>
          </div>
          ...
          <section className={styles.timelineSection} id="timeline">
            <SectionHeading
              eyebrow="02 / timeline e eventos"
              title="Fluxo de entrada e IA"
              copy="Registro dos eventos recebidos pelo canal WhatsApp vinculado exclusivamente ao seu negócio."
            />
            <div className={styles.timelineContainer}>
              {inboxEvents.length > 0 ? (
                  ...
                  {inboxEvents.map((ev) => (
                    <div className={styles.eventCard} key={ev.id}>
                      <div className={styles.eventHead}>
                        ...
                        <StatusPill tone={ev.status === 'PROCESSED' ? 'olive' : 'amber'}>
                          {ev.status}
                        </StatusPill>
                      </div>
                      <p className={styles.eventSender}>
                        Remetente: <strong>{ev.senderContact}</strong>
                      </p>
                ...
                <div className={styles.timelineEmptyBox}>
                  ...
                  <p>
                    Assim que o canal WhatsApp deste negócio estiver conectado, as novas conversas
                    aparecerão aqui.
                  </p>
                  <button
                    className={styles.inkButton}
                    onClick={() =>
                      toast('Verificação concluída', {
                        description: 'Webhook pronto para receber requisições da Meta Cloud API.',
                      })
                    }
                  >
                    verificar agora
          ...
          <section className={styles.scheduleSection} id="schedule">
            <div className={styles.scheduleCopy}>
              ...
              <p>
                O catálogo é configurado e persistido no configurador do negócio. A leitura
                operacional neste painel será conectada somente quando o contrato de agenda estiver
                disponível.
              </p>
              <a href="/" className={styles.inkButton}>
                abrir configurador <ArrowUpRight size={16} />
              </a>
            </div>
            ...
            <div className={styles.scheduleServicesBox}>
              ...
              <div className={styles.servicesList}>
                <div className={styles.servicesEmpty}>
                  ...
                  <span>
                    Nenhum serviço é mostrado aqui até existir uma leitura autenticada do catálogo.
                    Use o configurador para criar, editar ou inativar serviços.
                  </span>
                </div>
              </div>
            </div>
          ...
          <section className={styles.bottomGrid}>
            ...
            <div>
              <div className={styles.nextActionsHead}>
                <div>
                  <span className={styles.eyebrow}>caderno de campo</span>
              ...
              <button
                ...
                onClick={() =>
                  toast('Decisão 01', {
                    description:
                      'Envie uma primeira mensagem ao número do bot para gerar o evento real.',
                  })
                }
              >
                <span className={styles.actionNumber}>01</span>
                <span>
                  <strong>Enviar a primeira mensagem</strong>
                  <small>Validar webhook e inbox_events</small>
                </span>
              ...
              <button className={styles.actionRow} onClick={() => handleNav('schedule')}>
                ...
                <span>
                  <strong>Gerenciar serviços da agenda</strong>
                  <small>Alimentar os dados do salão</small>
                </span>
                <ArrowUpRight size={16} />
              </button>
              <button
                className={styles.actionRow}
                onClick={() =>
                  toast('Domínio eddigital.ia.br', {
                    description: 'Pronto para publicação via Vercel.',
                  })
                }
              >
                <span className={styles.actionNumber}>03</span>
                <span>
                  <strong>Vincular domínio eddigital.ia.br</strong>
                  <small>Publicar via painel de configurações</small>
                </span>
                <ArrowUpRight size={16} />
              </button>
            </div>
          ...
          <footer className={styles.pageFooter}>
            <span>
              <Zap size={13} /> {tenantName} / operação autenticada
            </span>
        ...
        <div className={styles.drawerBackdrop} onClick={() => setSelectedComponent(null)}>
          <aside className={styles.evidenceDrawer} onClick={(event) => event.stopPropagation()}>
            <div className={styles.drawerHead}>
              <div>
                <span className={styles.eyebrow}>evidência do componente</span>
            ...
            <div className={styles.drawerStatus}>
              <StatusPill tone={selectedComponent.tone}>{selectedComponent.status}</StatusPill>
              <span>{tenantName} / operação</span>
            </div>
            <p className={styles.drawerDetail}>{selectedComponent.detail}</p>
            <div className={styles.drawerNote}>
              <FileCheck2 size={17} />
              <p>{selectedComponent.evidence}</p>
            </div>
            <div className={styles.drawerMeta}>
              <span>
                <Terminal size={14} /> fonte registrada no Supabase
              </span>
              <span>
                <Inbox size={14} /> persistência ativa
              </span>
            </div>
            <button
              className={`${styles.inkButton} ${styles.drawerButton}`}
              onClick={() => setSelectedComponent(null)}
            >
              fechar
            </button>
